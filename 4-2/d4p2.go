...
func checkBoards(boards [][][]int, draw []int) (int, [][]int, []int) {
	...
	for _, board := range boards {
		boardWinTurn, boardDrawn := winturn(board)
		if boardWinTurn > losingTurn {
			losingTurn = boardWinTurn
			losingBoard = board
			losingDrawn = boardDrawn
			fmt.Println(boardWinTurn)
		}
	}

	return losingTurn, losingBoard, losingDrawn
}
